    ...
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="w-full">
          <div className="flex items-center justify-between">
            ...
            <Link 
              ...
              className="group flex items-center space-x-2 pl-6 md:pl-8 transition-all duration-300 hover:pl-7"
            >
              <span className="text-2xl md:text-3xl group-hover:scale-110 transition-transform duration-300">🍔</span>
              <span className={`text-lg md:text-xl font-bold transition-all duration-300 group-hover:scale-105 ${
            ...
            <div className="hidden md:flex items-center space-x-1 pr-6 md:pr-8">
              {navLinks.map((link) => (
                ...
                >
                  {link.name}
                  <span className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-orange-500 to-red-500 transition-all duration-300 ${
            ...
            <div className="flex items-center space-x-3 pr-6 md:pr-8">
              ...
              <button
                ...
                className={`relative p-2 rounded-full transition-all duration-300 hover:scale-110 ${
                  isDark ? 'bg-gray-800 text-yellow-500' : 'bg-gray-100 text-gray-700'
                }`}
              >
              ...
              {user ? (
                <div className="relative">
                  ...
                  >
                    ...
                    <i className="fas fa-chevron-down text-xs transition-transform duration-300"></i>
                  </button>
                  
                  {showUserMenu && (
                    <div className={`absolute right-0 mt-2 w-56 rounded-lg shadow-lg py-2 animate-fadeInUp ${
                      ...
                      <Link to="/account" className={`flex items-center space-x-3 px-4 py-2 text-sm transition-all duration-300 ${
                        isDark ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'
                      }`}>
                        <i className="fas fa-user-circle w-4"></i>
                        <span>My Account</span>
                      </Link>
                      <Link to="/orders" className={`flex items-center space-x-3 px-4 py-2 text-sm transition-all duration-300 ${
                        isDark ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'
                      }`}>
                      ...
                      {(user.email === 'admin@foodie.com' || user.role === 'admin') && (
                        <Link to="/admin" className={`flex items-center space-x-3 px-4 py-2 text-sm transition-all duration-300 ${
                          isDark ? 'text-gray-300 hover:bg-gray-700' : 'text-gray-700 hover:bg-gray-100'
                        }`}>
                      ...
                      <button onClick={handleLogout} className="flex items-center space-x-3 w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-all duration-300">
              ...
              >
                <div className="absolute inset-0 flex flex-col items-center justify-center space-y-1.5">
                  <span className={`w-5 h-0.5 transition-all duration-300 ${isDark ? 'bg-white' : 'bg-gray-800'} ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
                  <span className={`w-5 h-0.5 transition-all duration-300 ${isDark ? 'bg-white' : 'bg-gray-800'} ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
                  <span className={`w-5 h-0.5 transition-all duration-300 ${isDark ? 'bg-white' : 'bg-gray-800'} ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
                </div>
              </button>
            </div>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${
          ...
          <div className={`px-6 py-4 space-y-2 border-t ${isDark ? 'border-gray-700 bg-gray-900' : 'border-gray-100 bg-white'}`}>
            {navLinks.map((link) => (
              <button
                key={link.name}
                onClick={() => scrollToSection(link.id, link.link)}
                className={`w-full text-left px-4 py-3 rounded-lg text-sm font-medium transition-all duration-300 ${
                  activeLink === link.link
                    ? isDark ? 'bg-gray-800 text-orange-400' : 'bg-gray-100 text-orange-500'
      ...
      {showCartPanel && (
        <>
          ...
          <div className={`fixed right-0 top-0 h-full w-full max-w-md shadow-2xl z-50 transform transition-transform duration-300 animate-slideInRight ${
            isDark ? 'bg-gray-800' : 'bg-white'
          }`}>
            <div className="flex flex-col h-full">
              <div className={`flex justify-between items-center p-6 border-b ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
                <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>Your Cart</h2>
                <button onClick={() => setShowCartPanel(false)} className={`w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 hover:rotate-90 ${
              ...
              <div className="flex-1 overflow-y-auto p-6">
                {cartItems.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {cartItems.map(item => (
                      <div key={item.id} className={`flex gap-4 p-4 rounded-xl transition-all duration-300 hover:scale-105 ${
              ...
              {cartItems.length > 0 && (
                <div className={`border-t p-6 space-y-4 ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
